...
export async function createRegistration(formData: RegistrationFormData): Promise<string> {
  ...
  const rawPayload = {
    ...
    members: includedMembers.map((member) => ({
      ...
      career: formData.category === "universidades" ? member.career ?? "" : undefined,
      universityYear: formData.category === "universidades" ? member.universityYear ?? "" : undefined,
      schoolGrade: formData.category === "colegios" ? member.schoolGrade ?? "" : undefined,
      about: member.about ?? "",
